...
const updateTimer = function(timeLeft,data) {
  if (timeLeft > 0) {
    if (!success) {
      ...
      if (data) {
        if (data.http_code && data.error_description) {
    ...
    if (success) {
      let url;
      // in case quizlet changes how they send back the url of the new set (has happened before)
      if (data.url.includes("https://quizlet.com")) {
        url = data.url;
      } else {
        url = "https://quizlet.com" + data.url;
      }
