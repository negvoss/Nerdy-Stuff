...
window.onbeforeunload = function() {
  return("Are you sure?"); // on close/reload window
}

function preload() {
  transparent_background = loadImage("images/transparent_background.png");
  img = loadImage("images/example.jpg");
}
...
function draw() {
  drawImage();
  if (performingFloodFill) {
    noStroke();
    fill(255,100,0,100);
    for (var i = 0; i < floodFillPoints.length; i++) {
      ellipse(
        (floodFillPoints[i].x - imageXToCenter)*zoom + width/2,
        (floodFillPoints[i].y - imageYToCenter)*zoom + height/2,
        20
      )
    }
  }
