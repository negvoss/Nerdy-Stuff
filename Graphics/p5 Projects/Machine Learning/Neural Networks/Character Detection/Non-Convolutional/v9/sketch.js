...
function setup() {
  createP("This program differentiates between these characters: [" + possibleOutputs + "].");
  setupFirebase();
...
const setupControls = function() {
  ...
  let userAnswerLabel = select("#userAnswerLabel");
  userAnswer = select("#userAnswer").input(function() {
    userAnswerLabel.html(possibleOutputs[userAnswer.value()]);
  });
  userAnswer.elt.max = possibleOutputs.length - 1;
  userAnswerLabel.html(possibleOutputs[userAnswer.value()]);
}
